...
class Problem():
    ...
    def setup_parser(self):
        ...
        term.setParseAction(self.make_term_from_pargs)
        self.expr_parser = pyparsing.infixNotation(term,
                [
                ('=', 2, pyparsing.opAssoc.LEFT, self.make_assignment_from_pargs),
                ('~', 1, pyparsing.opAssoc.RIGHT, self.make_negation_from_pargs),
                ('&', 2, pyparsing.opAssoc.LEFT, self.make_conjunction_from_pargs),
                ('|', 2, pyparsing.opAssoc.LEFT, self.make_disjunction_from_pargs),
                ('^', 2, pyparsing.opAssoc.LEFT, self.make_xor_from_pargs),
                ('=>', 2, pyparsing.opAssoc.LEFT, self.make_implication_from_pargs),
                ('<=>', 2, pyparsing.opAssoc.LEFT, self.make_equivalence_from_pargs),
                ])

    def make_term_from_pargs(self, pargs):
        return Term(pargs[0])

    def make_assignment_from_pargs(self, pargs):
        return Assignment(pargs[0][0::2])

    def make_negation_from_pargs(self, pargs):
        return Negation(pargs[0][1])

    def make_conjunction_from_pargs(self, pargs):
        return Conjunction(pargs[0][0::2])

    def make_disjunction_from_pargs(self, pargs):
        return Disjunction(pargs[0][0::2])

    def make_xor_from_pargs(self, pargs):
        return Xor(pargs[0][0::2])

    def make_implication_from_pargs(self, pargs):
        return Implication(pargs[0][0::2])

    def make_equivalence_from_pargs(self, pargs):
